class Api::Auth::LoginController < Api::AuthController
  ...
  def password
    ...
    if customer.nil? || customer.auth_mode != Customer::AUTH_MODE['PASSWORD_AUTH']
      render status: 404, json: { success: false, message: I18n.t('auth.login_failed'), 
        reason: { key => [ I18n.t('auth.user.not_found') ] } }
      return
    elsif customer.is_blocked?
      render status: 401, json: { success: false, message: I18n.t('auth.login_failed'), 
        reason: { key => [ I18n.t('auth.account_blocked', platform: PlatformConfig['name']) ] } }
      return
  ...
  def google
    ...
    if customer.nil? || (!PlatformConfig['flexible_auth'] && customer.auth_mode != Customer::AUTH_MODE['GOOGLE_AUTH'])
      render status: 404, json: { success: false, message: I18n.t('auth.login_failed'), 
        reason: { email: [ I18n.t('auth.user.not_found') ] } }
      return
    elsif customer.is_blocked?
      render status: 400, json: { success: false, message: I18n.t('auth.login_failed'), 
        reason: { email: [ I18n.t('auth.account_blocked', platform: PlatformConfig['name']) ] } }
      return
    end
