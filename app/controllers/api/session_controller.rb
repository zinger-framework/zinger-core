class Api::SessionController < ApiController
  def index
    ...
    render status: 200, json: { success: true, message: 'success', data: { sessions: sessions } }
  end

  def destroy
    session = Customer.current.customer_sessions.find_by_token(params['id'])
    if session.blank?
      render status: 404, json: { success: false, message: I18n.t('session.delete_failed'), reason: I18n.t('session.not_found') }
      return
    end

    if params['id'] == CustomerSession.extract_token(request.headers['Authorization'])
      render status: 400, json: { success: false, message: I18n.t('session.delete_failed'), reason: I18n.t('session.current_delete_failed') }
      return
    end
